...
class KeywordExtraction(object):
	...
	# print word_scores
        sorted_keywords = sorted(word_scores.items(), key=operator.itemgetter(1), reverse=True)
        return sorted_keywords[:int(len(sorted_keywords)/3)]
...
def extract_keywords(domain,prodID):
    ...
    for i in review_data:
        text+=i["review"]
    extraction_instance = KeywordExtraction(STOP_WORD_PATH+"stoplist.txt")
    keywords_data_score = extraction_instance.run(text)
    keywords = [i[0] for i in keywords_data_score]
    return keywords

def extract_keywords_custom(domain,prodID,brands_reviews):
	review_data = brands_reviews[prodID]
	text = ""
	for i in review_data:
		text+=i["review"]
